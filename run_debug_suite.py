#!/usr/bin/env python3
"""
Debug script to run all experiment configurations with limited epochs (5) to verify pipeline.

This script runs one epoch-limited trial for every experiment configuration to ensure
the pipeline is crash-free before running full experiments.
"""
...
def run_experiment(cmd: List[str], exp_name: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        error_msg = f"Experiment timed out after 10 minutes"
...
def main():
    """Main function to run all debug experiments."""
    
    # Global debug settings
    DEBUG_EPOCHS = 5
    DEBUG_BATCH_SIZE = 32  # Keep original batch size for compatibility
    
    # Base command (add --no_plot to skip plotting)
    # Device selection is automatic: CUDA > MPS > CPU (fastest available)
    base_cmd = ["python", "main.py"]
    
    # Experiment configurations
    # Only one configuration per algorithm per dataset (12 experiments total)
    experiments = []
    
    # ===== Group 1: CIFAR-100 (40% Label Noise) =====
    base_cifar = ["--task", "cifar100", "--epochs", str(DEBUG_EPOCHS)]
    
    experiments.extend([
        (base_cifar + ["--algorithm", "standard"], "CIFAR-100 - Standard SGD"),
        (base_cifar + ["--algorithm", "mkl", "--k_ratio", "2.0"], "CIFAR-100 - MKL-SGD"),
        (base_cifar + ["--algorithm", "rho", "--selection_ratio", "0.1"], "CIFAR-100 - RHO-LOSS"),
        (base_cifar + ["--algorithm", "hasa", "--window_size", "5", "--selection_ratio", "0.6"], 
         "CIFAR-100 - HASA"),
    ])
    
    # ===== Group 2: MNIST (Input Noise std=1.5) =====
    base_mnist = ["--task", "mnist", "--epochs", str(DEBUG_EPOCHS)]
    
    experiments.extend([
        (base_mnist + ["--algorithm", "standard"], "MNIST - Standard SGD"),
        (base_mnist + ["--algorithm", "mkl", "--k_ratio", "2.0"], "MNIST - MKL-SGD"),
        (base_mnist + ["--algorithm", "rho", "--selection_ratio", "0.1"], "MNIST - RHO-LOSS"),
        (base_mnist + ["--algorithm", "hasa", "--window_size", "5", "--selection_ratio", "0.6"], 
         "MNIST - HASA"),
    ])
    
    # ===== Group 3: CLOUD (Input Noise std=1.0) =====
    base_cloud = ["--task", "cloud", "--epochs", str(DEBUG_EPOCHS)]
    
    experiments.extend([
        (base_cloud + ["--algorithm", "standard"], "CLOUD - Standard SGD"),
        (base_cloud + ["--algorithm", "mkl", "--k_ratio", "2.0"], "CLOUD - MKL-SGD"),
        (base_cloud + ["--algorithm", "rho", "--selection_ratio", "0.1"], "CLOUD - RHO-LOSS"),
    ...
    # Run all experiments
    print_colored(f"\n{'='*80}", BOLD)
    print_colored(f"DEBUG SUITE: Running {len(experiments)} experiments with {DEBUG_EPOCHS} epochs each", BOLD)
    print_colored(f"{'='*80}\n", BOLD)
    ...
    # Summary
    print_colored(f"\n{'='*80}", BOLD)
    print_colored("DEBUG SUITE SUMMARY", BOLD)
    print_colored(f"{'='*80}\n", BOLD)
